...
#### Scenario: Expose official turnover and pytdx breadth outcome
- **WHEN** turnover resolves through official exchange sources and rise-fall statistics resolve through `pytdx`, or either metric falls back to a legacy path
- **THEN** stage 1 output SHALL clearly reflect the width-data source outcome
- **AND** users SHALL be able to distinguish official/pytdx primary success, legacy fallback success, and degraded empty-value failure
