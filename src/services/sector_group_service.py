...
# 板块主题定义（概念层）。
# 注：task 9 板块口径切换到申万一级行业(SW1)后，tracked_sectors 是 SW1 行业（医药生物/银行等），
# 这些东财概念主题(人形机器人链/光伏产业链等)与之口径不同，主题匹配在真实 SW1 数据下弱化。
# 保留定义以维持主题框架与测试，后续可按 SW1 行业重新定义（或 config/sector_group_themes.json 覆盖）。
