...
class CLSRollClient:
    ...
    def _fetch_with_httpx(self, params: dict[str, str]) -> Optional[dict]:
        ...
        try:
            ...
            with httpx.Client(timeout=self.timeout, trust_env=False, verify=False) as client:
                response = client.get(CLS_ROLL_URL, params=params, headers=headers)
    ...
    def _fetch_with_curl(self, params: dict[str, str]) -> Optional[dict]:
        ...
        try:
            ...
            url = f"{CLS_ROLL_URL}?{query_string}"
    ...
    def _fetch_page(self, last_time: int, rn: int = DEFAULT_PAGE_SIZE) -> list[dict[str, Any]]:
        """获取单页数据。
        ...
        params = self._build_params(last_time, rn)
    ...
    def fetch_latest(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        return self._fetch_page(current_time, rn=limit)
    ...
    def fetch_by_time_range(
        ...
        while current_time > start_time:
            items = self._fetch_page(current_time, rn=self.DEFAULT_PAGE_SIZE)
    ...
    def parse_telegraph(self, item: dict[str, Any]) -> dict[str, Any]:
        """解析单条电报数据。
