...
# ── 5.2 _ensure_today_batch ───────────────────────────────────


class TestEnsureTodayBatch:
    """测试 batch 创建、恢复和补充逻辑。"""
    ...
    async def test_creates_batch_for_all_active_feeds(
        ...
        await fetcher._ensure_today_batch()

        assert len(added_records) == 3
        for rec in added_records:
            assert rec.status == "pending"
            assert rec.batch_date == date.today()
...
class TestGetPendingFeeds:
    ...
    async def test_returns_only_pending_feeds(
        ...
        result = await fetcher._get_pending_feeds()
        assert len(result) == 1
...
class TestFetchAllResume:
    ...
    async def test_rate_limit_keeps_feed_pending(
        ...
        with patch.object(fetcher, "_ensure_today_batch", new_callable=AsyncMock), \
    ...
    async def test_success_marks_done(
        ...
        with patch.object(fetcher, "_ensure_today_batch", new_callable=AsyncMock), \
            ...
            assert "A" in results
            mock_done.assert_called_once_with("A")
...
class TestForceReset:
    ...
    async def test_force_calls_reset_before_ensure(
        ...
        """force=True 时先调用 _reset_today_batch 再调用 _ensure_today_batch。"""
        call_order: list[str] = []
        ...
        feeds = [_make_feed("A")]
        with patch.object(fetcher, "_reset_today_batch", new_callable=AsyncMock, side_effect=_mock_reset), \
             patch.object(fetcher, "_ensure_today_batch", new_callable=AsyncMock, side_effect=_mock_ensure), \
             patch.object(fetcher, "_get_pending_feeds", new_callable=AsyncMock, return_value=feeds), \
             patch.object(fetcher, "_fetch_incremental_or_init_summary", new_callable=AsyncMock, return_value=FetchSummary(mp_id="A")), \
             patch.object(fetcher, "_mark_batch_done", new_callable=AsyncMock):
    ...
    async def test_no_force_skips_reset(
        ...
        """force=False 时不调用 _reset_today_batch。"""
        feeds = [_make_feed("A")]
        with patch.object(fetcher, "_reset_today_batch", new_callable=AsyncMock) as mock_reset, \
             patch.object(fetcher, "_ensure_today_batch", new_callable=AsyncMock), \
             patch.object(fetcher, "_get_pending_feeds", new_callable=AsyncMock, return_value=feeds), \
             patch.object(fetcher, "_fetch_incremental_or_init_summary", new_callable=AsyncMock, return_value=FetchSummary(mp_id="A")), \
             patch.object(fetcher, "_mark_batch_done", new_callable=AsyncMock):
...
class TestOldBatchCleanup:
    ...
    async def test_cleanup_deletes_old_records(
        ...
        """_ensure_today_batch 应调用 DELETE 清理 7 天前的记录。"""
...
class TestSingleFetchUnaffected:
    ...
    async def test_single_fetch_does_not_use_batch(
        ...
        with patch.object(fetcher, "_get_feed_or_raise", new_callable=AsyncMock, side_effect=ValueError("中断")):
            with patch.object(fetcher, "_ensure_today_batch", new_callable=AsyncMock) as mock_ensure, \
